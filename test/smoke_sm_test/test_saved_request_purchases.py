...
def test_sm_saved_request_monitoring_on_purchases_page(app):
    i1 = "Перейти в сохраненные запросы"
    reestr_ex = "Торги"
    text = "Запрос по странице торги %s"
    ...
    app.session.ensure_login_sm(app.username, app.password)
    app.session.open_SM_page(app.smPurchases)
    #find_in_container_number(11 - всего контейнеров + 1, 0 -номер нужного контейнера если 0 то случайный выбор контейнера)
    s = app.testhelpersm.find_in_container_number(11, 0)
