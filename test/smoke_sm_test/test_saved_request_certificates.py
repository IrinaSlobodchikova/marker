...
def test_sm_saved_request_monitoring_on_certificates_page(app):
    i1 = "Перейти в сохраненные запросы"
    reestr_ex = "Сертификаты"
    text = "Запрос по странице сертификаты %s"
    ...
    app.session.ensure_login_sm(app.username, app.password)
    app.session.open_SM_page(app.smCertificates)
    #find_in_container_number(всего контейнеров + 1, номер нужного контейнера (если 0 то случайный выбор контейнера))
    s = app.testhelpersm.find_in_container_number(5, 0)
